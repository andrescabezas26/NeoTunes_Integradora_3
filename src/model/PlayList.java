...
public class PlayList {
    ...
    /**
     * initMatrixCode= This method init with random numbers (from 1 to 9) the matrix
     * of the playlist
     */
    ...
    /**
     * createCodeTypePlaylist= This method creates a code for the playlist according
     * of his type of playlist
     */
    ...
    public int searchAudioByProducerAndName(String name, String producerNickName) {
        ...
        for (int i = 0; i < audios.size() && !sameSong; i++) {
            if (audios.get(i).getName().equalsIgnoreCase(name)) {
                if ((audios).get(i).getCreator().equals(producerNickName)) {
                    sameSong = true;
                    pos = i;
                }
            }
    ...
    public String showAudio() {
        String msj = "Upss, esta playlist esta vacia";
        if (audios.size() != 0) {
    ...
    /**
     * playAudio= This method implements Iplayable and increase the audio playbacks
     * @param audioPos: int = The position of the audio in the playlist
     * @return msj: String = The Iplayable interface method play of the audio
     */
    public String playAudio(int audioPos) {
        String msj = "";
        if (audios.get(audioPos) instanceof Song) {
            msj = ((Song) audios.get(audioPos)).play();
            ((Song) audios.get(audioPos)).setPlaybacks(((Song) audios.get(audioPos)).getPlaybacks() + 1);
        } else {
            msj = ((Podcast) audios.get(audioPos)).play();
            ((Podcast) audios.get(audioPos)).setPlaybacks(((Podcast) audios.get(audioPos)).getPlaybacks() + 1);
        }
        return msj;
    }
